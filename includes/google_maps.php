...
?>
	...
	<?php
				...
				$db->query("SELECT serverId, IF(publicaddress != '', publicaddress, CONCAT(address, ':', port)) AS addr, name, kills, lat, lng, city, country FROM hlstats_Servers WHERE game='".$db->escape($game)."' AND lat IS NOT NULL AND lng IS NOT NULL");
				...
				$db->query("SELECT 
						...
						WHERE 
							...
							AND hlstats_Servers.game='".$db->escape($game)."'
							");
				$players = array();
				while ($row = $db->fetch_array())
				{
