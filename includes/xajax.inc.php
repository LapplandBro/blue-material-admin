...
class xajax
{
	...
	{	
		...
		// CSRF для state-changing xajax. Read-only опрос серверов — без токена:
		// MOTD/оверлей в игре часто не шлёт куки сессии (SameSite) → иначе 500/алерт в TF2.
		$csrf_exempt = array(
			'ServerHostPlayers',
			'ServerHostProperty',
		...
		if (function_exists('sb_csrf_validate') && !in_array($sFunctionName, $csrf_exempt, true)) {
			...
			if (!sb_csrf_validate($token)) {
				// Не через xajaxResponse: при битой сессии/кодировке getXML мог отдавать пустой 500.
				if (!headers_sent()) {
					header('HTTP/1.1 200 OK');
					header('Content-Type: text/xml; charset=utf-8');
				}
				$msg = 'Session expired or invalid CSRF token. Refresh the page (Ctrl+F5) and try again.';
				echo '<?xml version="1.0" encoding="utf-8" ?><xjx><cmd n="al"><![CDATA[' . $msg . ']]></cmd></xjx>';
				exit();
			}
		}
		...
		if ($bFoundFunction)
		{
			...
			{
				// Magic quotes removed in PHP 5.4+; no stripslashes needed.
				if (!is_string($aArgs[$i])) {
					$aArgs[$i] = (string)$aArgs[$i];
				}
	...
	function getJavascriptInclude($sJsURI="", $sJsFile=NULL)
	{
		...
		$html = "\t<script src=\"" . $sJsURI . $sJsFile . "\"></script>\n";
		if ($this->iTimeout != 0)
		{
			$html .= "\t<script>\n";
			$html .= "window.setTimeout(function () { if (!xajaxLoaded) { if(typeof ShowBox==='function'){ShowBox('Ошибка','Не удалось загрузить xajax Javascript.\\nURL: {$sJsURI}{$sJsFile}','red','',true);}else{alert('Error: the xajax Javascript file could not be included. Perhaps the URL is incorrect?\\nURL: {$sJsURI}{$sJsFile}');} } }, ".$this->iTimeout.");\n";
			$html .= "\t</script>\n";
		}
		return $html;
	}
	...
	function _parseObjXml($rootTag)
	{
		...
		if ($rootTag == "xjxquery")
		{
			...
			// Magic quotes removed in PHP 5.4+; parse_str no longer adds slashes.
		}
