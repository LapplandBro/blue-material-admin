...
class xajaxResponse
{
	...
	/**
	 * Показывает сообщение через ShowBox сайта (не браузерный alert).
	 *
	...
	function addAlert($sMsg)
	{
		...
		$js = 'if(typeof ShowBox==="function"){ShowBox('
			...
			. ',' . json_encode($html, JSON_UNESCAPED_UNICODE)
			. ',' . json_encode($color)
			. ',"",true);}else{alert(' . json_encode($msg, JSON_UNESCAPED_UNICODE) . ');}';
