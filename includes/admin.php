...
else 
{
	...
	if($_GET['c'] == "groups")
	{
		...
		{
			$groupsTabMenu = new CTabsMenu();
			$groupsTabMenu->addMenuItem("Назад",0, "", "index.php?p=admin&c=groups", true);
			$groupsTabMenu->outputMenu();
	 ...
	 // ###################[ Admins ]##################################################################
	{
					...
					case "admwebflag":
						...
						while(!$alladmins->EOF)
						{
							if($userbank->HasAccess($flagstring, $alladmins->fields["aid"])) {
								$accessaid .= ",".(int)$alladmins->fields["aid"];
							}
							$alladmins->MoveNext();
						}
		...
		{
			$adminTabMenu = new CTabsMenu();
			$adminTabMenu->addMenuItem("Назад", 0,"", "index.php?p=admin&c=admins", true);
			$adminTabMenu->outputMenu();
	 ...
	 // ###################[ Servers ]##################################################################
	{
		...
		{
			$serverTabMenu = new CTabsMenu();
			$serverTabMenu->addMenuItem("Назад", 0,"", "index.php?p=admin&c=servers", true);
			$serverTabMenu->outputMenu();
	 ...
	 // ###################[ Recidivism / violation history ]##################################################################
	{
		...
		RewritePageTitle("История нарушений");
	}
	 ...
	 // ###################[ PARSEC / fingerprint panel ]##################################################################
	{
		...
		RewritePageTitle("Связанные аккаунты");
	}
	elseif($_GET['c'] == "mods")
	 // ###################[ Mods ]##################################################################
	{
