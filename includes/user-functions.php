 ...
 */
function logout() {
	$aid = isset($_COOKIE['aid']) ? (int)$_COOKIE['aid'] : 0;
	$tok = isset($_COOKIE['password']) ? (string)$_COOKIE['password'] : '';
	$ok = ($aid > 0 && $tok !== '' && function_exists('sb_verify_web_session') && sb_verify_web_session($aid, $tok));
	if ($ok && function_exists('sb_clear_web_session'))
...
function userdata($aid, $pass)
{
    global $userbank;
    $aid = (int)$aid;
    // $pass здесь — уже хэш из сессии/куки, не plaintext.
    if(!$userbank->CheckLogin($pass, $aid))
    {
...
function get_user_admin($steam)
{
	if(empty($steam))
		return 0;
	$steamq = $GLOBALS['db']->qstr($steam);
	$admin = $GLOBALS['db']->GetRow("SELECT * FROM " . DB_PREFIX . "_admins WHERE authid = " . $steamq);
	if(strlen($admin['srv_group']) > 1)
	{
		$query = $GLOBALS['db']->GetRow("SELECT flags FROM " . DB_PREFIX . "_srvgroups WHERE name = (SELECT srv_group FROM " . DB_PREFIX . "_admins WHERE authid = " . $steamq . ")");
...
function get_non_inherited_admin($steam)
{
	if(empty($steam))
		return 0;
	$admin = $GLOBALS['db']->GetRow("SELECT * FROM `" . DB_PREFIX . "_admins` WHERE authid = " . $GLOBALS['db']->qstr($steam));
	return $admin['srv_flags'];
}
