...
class CDonate {
     ...
     * @return int
     */
    public function AddPayment_Admin($name, $authid, $tariff, $vk = '', $discord = '') {
        if (!$this->IsTariffExists($tariff))
            return -1;
        
        $query = sprintf("INSERT INTO `%s_billing_adminpayments` (`name`, `authid`, `tariff`, `vk`, `discord`) VALUES (%s, %s, %d, %s, %s);", DB_PREFIX, $GLOBALS['db']->qstr($name), $GLOBALS['db']->qstr($authid), (int) $tariff, $GLOBALS['db']->qstr($vk), $GLOBALS['db']->qstr($discord));
        $GLOBALS['db']->Execute($query);
        return $GLOBALS['db']->Insert_ID();
    }
