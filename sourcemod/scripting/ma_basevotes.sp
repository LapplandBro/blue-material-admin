...
void DisplayBanTargetMenu(int client)
{
	...
	menu.SetTitle(title);
	if (GetMenuItemPlayers(client, menu, true, true, false))
	{
...
void DisplayKickTargetMenu(int client)
{
	...
	menu.SetTitle(title);
	if (GetMenuItemPlayers(client, menu, true, true))
	{
		menu.ExitBackButton = true;
		menu.Display(client, MENU_TIME_FOREVER);
	}
...
bool GetMenuItemPlayers(int client, Menu & menu, bool showbot, bool showdead, bool sourcetv = true)
{
	EngineVersion nEngine = GetEngineVersion();
	char sBuffer[2][128];
	int iUserId[MAXPLAYERS+1], iArray[MAXPLAYERS] = {0, ...}, iSize = 0, iTeam;
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientConnected(i))
		{
			iUserId[i] = GetClientUserId(i);
			if (iUserId[i] != 0 && IsClientInGame(i))
			{
				if ((!showbot && IsFakeClient(i)) || (!showdead && !IsPlayerAlive(i)) || (!sourcetv && IsClientSourceTV(i)))
				{
					continue;
				}
		...
		for (int j = 1; j <= MaxClients; j++)
		{
			if (iArray[i] == iUserId[j])
			{
				if (!IsClientInKickQueue(j))
				{
					...
					if (nEngine == Engine_SourceSDK2006 || nEngine == Engine_CSS || nEngine == Engine_CSGO)
					{
						iTeam = GetClientTeam(j);
						if (iTeam < 2)
						{
							Format(sBuffer[1], sizeof(sBuffer[]), "[SPEC] %s #%s", sBuffer[1], sBuffer[0]);
						}
						else if (iTeam == 2)
						{
							Format(sBuffer[1], sizeof(sBuffer[]), "[T] %s #%s", sBuffer[1], sBuffer[0]);
						}
						else
						{
							Format(sBuffer[1], sizeof(sBuffer[]), "[CT] %s #%s", sBuffer[1], sBuffer[0]);
						}
					}
					else
					{
						Format(sBuffer[1], sizeof(sBuffer[]), "%s #%s", sBuffer[1], sBuffer[0]);
					}
					menu.AddItem(sBuffer[0], sBuffer[1]);
				}
				break;
			}
		}
			...
			switch (g_voteType)
			{
				...
				case (ban):
				{
					...
					else
					{
						...
						MABanPlayer(GetClientOfUserId(g_iCallerUserId), voteTarget, MA_BAN_STEAM, 30, "Забанен голосованием админа");
					}
				}
			}
		}
