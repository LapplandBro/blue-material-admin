...
public void OnClientAuthorized(int iClient, const char[] szEngineAuthId)
{
	...
	// MA stores / matches admins by SteamID suffix (authid[8] → "Y:ZNNNN"), not full STEAM_X:Y:Z
	char szAuthSuffix[64];
	strcopy(szAuthSuffix, sizeof(szAuthSuffix), szEngineAuthId[8]);
	...
	g_hDatabase.Format(szQuery, sizeof(szQuery), "\
		...
		WHERE \
			...
			AND (`expires` > UNIX_TIMESTAMP() OR `expires` = 0);", g_szDatabasePrefix, g_szDatabasePrefix, g_szDatabasePrefix, g_szDatabasePrefix, g_szDatabasePrefix, g_iServerID, g_szDatabasePrefix, g_iServerID, szAuthSuffix);
