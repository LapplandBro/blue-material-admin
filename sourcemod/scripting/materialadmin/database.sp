	...
	if (dbRs.HasResults && dbRs.RowCount && dbRs.FetchRow())
	{
 		...
 		if (!dbRs.IsFieldNull(0))
		{
			...
			if(g_bBanSayPanel && g_iGameTyp != GAMETYP_CSGO)
				CreateTeaxtDialog(iClient, 0.2, "%T", "Banned Admin panel", iClient, sAdmin, szReason, sCreated, sEnds, sLength, g_sWebsite);
			else
		...
		{
			if(g_bBanSayPanel && g_iGameTyp != GAMETYP_CSGO)
				CreateTeaxtDialog(iClient, 0.2, "%T", "Banned panel", iClient, szReason, sCreated, sEnds, sLength, g_sWebsite);
			else
