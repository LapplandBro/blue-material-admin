...
//--------------------------------------------------------------------------------------------------
bool MA_CanKickClient(int iClient)
{
	return iClient > 0
		&& iClient <= MaxClients
		&& IsClientConnected(iClient)
		&& IsClientInGame(iClient)
		&& !IsClientInKickQueue(iClient);
}
...
void CreateSayBanned(char[] sAdminName, int iClient, int iCreated, int iTime, char[] sLength, char[] sReason)
{
	...
	if (MA_CanKickClient(iClient))
		KickClient(iClient, "%T", "Banned Admin", iClient, sAdminName, sReason, sCreated, sLength, g_sWebsite);
}

void CreateTeaxtDialog(int iClient, float flKickDelay, const char[] sMesag, any ...)
{
	char sTitle[125],
		sText[1025];
	VFormat(sText, sizeof(sText), sMesag, 4);
	...
	{
	#if MADEBUG
		LogToFile(g_sLogAction, "CreateTeaxtDialog %N delay=%.2f", iClient, flKickDelay);
	#endif
		...
		if (flKickDelay <= 0.0)
		{
			...
			CreateTimer(flKickDelay, TimerKick, GetClientUserId(iClient));
	}
	delete kvKey;
}
...
{
	int iClient = GetClientOfUserId(iUserId);
	// After RemoveEntity crash the slot may be gone — do not KickClient.
	if (MA_CanKickClient(iClient))
		KickClient(iClient, "%T", "Banneds", iClient);
