...
.blockit-wrap { padding: 4px 2px 8px; }
.blockit-title { margin: 0 0 10px; font-size: 13px; font-weight: 600; color: #7ea8d4; }
.blockit-list { width: 100%; border-collapse: collapse; }
.blockit-list tr + tr td { border-top: 1px solid rgba(30, 144, 255, 0.12); }
.blockit-list td { padding: 7px 4px; vertical-align: top; }
.blockit-host { color: #e8f0ff; font-size: 12px; font-weight: 500; word-break: break-word; padding-right: 10px; }
.blockit-status { color: #7ea8d4; font-size: 12px; white-space: nowrap; text-align: right; }
...
.blockit-foot {
	...
	font-size: 13px; font-weight: 600; color: #81c784; display: none;
}
</style>
...
function set_counter(count)
{
	...
	if(srvcount==-{$total}- || count=='-1') {
		...
		if (foot) {
			...
			foot.textContent = (count == '-1') ? 'Игрок найден.' : 'Обход серверов завершён.';
		}
		try {
		...
		setTimeout(function(){ parent.document.location='../index.php?p=admin&c=bans'; }, 3500);
	}
	resizeFrame();
}
...
<div id="container" class="blockit-wrap" name="container">
	...
	<tr>
		<td><div class="blockit-host" id="srvip_-{$serv.num}-">-{$serv.ip}-:-{$serv.port}-</div></td>
		<td><div class="blockit-status" id="srv_-{$serv.num}-"><span class="muted">ждём…</span></div></td>
	</tr>
	-{/foreach}-
	</table>
