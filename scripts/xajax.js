...
var uri=xajaxRequestUri;var value;var sbCsrf=(typeof window.SB_CSRF==='string')?window.SB_CSRF:'';switch(xajaxRequestType){case xajaxDefinedGet:{var uriGet=uri.indexOf("?")==-1?"?xajax="+encodeURIComponent(sFunction):"&xajax="+encodeURIComponent(sFunction);if(aArgs){for(i=0;i<aArgs.length;i++){value=aArgs[i];if(typeof(value)=="object")
value=this.objectToXML(value);uriGet+="&xajaxargs[]="+encodeURIComponent(value);}
}
uriGet+="&xajaxr="+new Date().getTime();if(sbCsrf)uriGet+="&sb_csrf="+encodeURIComponent(sbCsrf);uri+=uriGet;postData=null;}break;case xajaxDefinedPost:{postData="xajax="+encodeURIComponent(sFunction);postData+="&xajaxr="+new Date().getTime();if(sbCsrf)postData+="&sb_csrf="+encodeURIComponent(sbCsrf);if(aArgs){for(i=0;i <aArgs.length;i++){value=aArgs[i];if(typeof(value)=="object")
value=this.objectToXML(value);postData=postData+"&xajaxargs[]="+encodeURIComponent(value);}
}
}break;default:
alert("Illegal request type: "+xajaxRequestType);return false;break;}
r=this.getRequestObject();if(!r)return false;r.open(xajaxRequestType==xajaxDefinedGet?"GET":"POST",uri,true);if(xajaxRequestType==xajaxDefinedPost){try{r.setRequestHeader("Method","POST "+uri+" HTTP/1.1");r.setRequestHeader("Content-Type","application/x-www-form-urlencoded");if(sbCsrf)r.setRequestHeader("X-SB-CSRF",sbCsrf);}
catch(e){alert("Your browser does not appear to  support asynchronous requests using POST.");return false;}
}
r.onreadystatechange=function(){if(r.readyState!=4)
return;if(r.status==200){if(xajaxDebug)xajax.DebugMessage("Received:\n"+r.responseText);if(r.responseXML&&r.responseXML.documentElement)
xajax.processResponse(r.responseXML);else{var _xr=String(r.responseText==null?"":r.responseText);var _xrShow=_xr.length>800?_xr.substr(0,800)+"\n… [ответ обрезан, "+_xr.length+" символов]":_xr;var errorString="Error: the XML response that was returned from the server is invalid.";errorString+="\nReceived:\n"+_xrShow;trimmedResponseText=_xr.replace(/^\s+/g,"");trimmedResponseText=trimmedResponseText.replace(/\s+$/g,"");if(trimmedResponseText!=_xr)
errorString+="\nYou have whitespace in your response.";ShowBox('AJAX Call Failed!', errorString.replace(/\n/g,"<br>"), 'red', '', true);if(document.getElementById('cmd')){document.getElementById('cmd').disabled=false;document.getElementById('cmd').value='';}if(document.getElementById('rcon_btn'))document.getElementById('rcon_btn').disabled=false;document.body.style.cursor='default';if(xajaxStatusMessages==true)window.status='Invalid XML response error';}
}
else{if(xajax.arrayContainsValue(xajax.responseErrorsForAlert,r.status)){var _xr2=String(r.responseText==null?"":r.responseText);var _xr2Show=_xr2.length>800?_xr2.substr(0,800)+"\n… [ответ обрезан, "+_xr2.length+" символов]":_xr2;var errorString="Error: the server returned the following HTTP status: "+r.status;errorString+="\nReceived:\n"+_xr2Show;if(typeof ShowBox==="function"){ShowBox('AJAX Call Failed!', errorString.replace(/\n/g,"<br>"), 'red', '', true);}else{alert(errorString);}}
if(document.getElementById('cmd')){document.getElementById('cmd').disabled=false;document.getElementById('cmd').value='';}if(document.getElementById('rcon_btn'))document.getElementById('rcon_btn').disabled=false;document.body.style.cursor='default';if(xajaxStatusMessages==true)window.status='Invalid XML response error';}
delete r;r=null;}
...
if(cmd=="al"){cmdFullname="addAlert";(function(m){m=String(m==null?"":m);var t="Сообщение",c="blue";if(/ошибк|error|запрещ|не найден|нельзя|отказано|выключено/i.test(m)){t="Ошибка";c="red";}else if(/успешн|изменен|добавлен|готово/i.test(m)){t="Успех";c="green";}if(typeof ShowBox==="function"){ShowBox(t,m.replace(/\n/g,"<br>"),c,"",true);}else{alert(m);}})(data);}
