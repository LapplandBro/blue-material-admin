...
{
	echo '<script>ShowBox("Ошибка", "Блокировка не найдена!", "red", "index.php?p=admin&c=comms");</script>';
	PageDie();
}

$canEditComms = $userbank->HasAccess(ADMIN_OWNER|ADMIN_EDIT_ALL_BANS)
...
if(isset($_POST['name']))
{
	...
	if($error == 0)
	{
		...
		if($edit)
		{
			...
			$log = new CSystemLog("m", "Блокировка отредактирована", $userbank->GetProperty("user") . " отредактировал блокировку №" . (int)$_GET['id'] . (!empty($changes) ? " (" . implode("; ", $changes) . ")" : " (без изменения полей)") . ".");
		...
		else
		{
			...
			echo '<script>ShowBox("Ошибка", "Не удалось обновить блокировку!' . (!empty($db_error) ? ' (' . addslashes(htmlspecialchars($db_error)) . ')' : '') . '", "red", "index.php?p=commslist'.$pagelink.'");</script>';
		}
	}
}
...
selectLengthTypeReason('<?php echo (int)$res['length']; ?>', '<?php echo (int)$res['type']-1; ?>', '<?php echo htmlspecialchars(addslashes($res['reason'])); ?>');
</script>
