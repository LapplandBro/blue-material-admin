...
{
	// SECURITY: use strict comparison to avoid PHP loose-comparison "magic hash" type-juggling bypass
	...
	$redir = "index.php?p=commslist".$pagelink;
	...
	if(empty($row) || !$row)
	{
		sb_list_action_done("Игроку не включен чат", "Игроку не был включен чат. Либо он был включен ранее, либо некорректный блок чата.", "red", $redir);
	}

	$unbanReason = htmlspecialchars(trim(isset($_GET['ureason']) ? $_GET['ureason'] : ''));
	...
	RecidivismRevokeOnUnpunish($row['authid'], 'gag', 'comms', $bid, 'web_ungag');

	$blocked = $GLOBALS['db']->GetAll("SELECT sid FROM `".DB_PREFIX."_servers` WHERE `enabled`=1");
	foreach($blocked as $tempban)
	{
		// Material Admin: ma_wb_unmute <type> <steam>  (2 = gag)
		SendRconSilent(("ma_wb_unmute 2 " . $row['authid']), $tempban['sid']);
	}

	if($ins){
		$log = new CSystemLog("m", "Игроку включен чат", "'".StripQuotes($row['name'])."' (" . $row['authid'] . ") has been ungagged");
		sb_list_action_done("Включение чата", "Игроку <b>".htmlspecialchars(StripQuotes($row['name']), ENT_QUOTES, 'UTF-8')."</b> (<b>" . htmlspecialchars($row['authid'], ENT_QUOTES, 'UTF-8') . "</b>) был включен чат.", "green", $redir);
	}
	sb_list_action_done("Чат не включен", "Возникла ошибка включения чата игроку <b>".htmlspecialchars(StripQuotes($row['name']), ENT_QUOTES, 'UTF-8')."</b>", "red", $redir);
}
else if(isset($_GET['a']) && $_GET['a'] == "unmute" && isset($_GET['id']))
{
	...
	if(empty($row) || !$row)
	{
		sb_list_action_done("Игроку не включен микро", "Игроку не был включен микрофон. Он либо был включен ранее, либо некорректный блок микрофона.", "red", $redir);
	}
	...
	RecidivismRevokeOnUnpunish($row['authid'], 'mute', 'comms', $bid, 'web_unmute');

	$blocked = $GLOBALS['db']->GetAll("SELECT sid FROM `".DB_PREFIX."_servers` WHERE `enabled`=1");
	foreach($blocked as $tempban)
	{
		// Material Admin: ma_wb_unmute <type> <steam>  (1 = mute)
		SendRconSilent(("ma_wb_unmute 1 " . $row['authid']), $tempban['sid']);
	}

	if($ins){
		$log = new CSystemLog("m", "Игроку влючен микро", "'".StripQuotes($row['name'])."' (" . $row['authid'] . ") has been unmuted");
		sb_list_action_done("Включение микрофона", "Игроку <b>".htmlspecialchars(StripQuotes($row['name']), ENT_QUOTES, 'UTF-8')."</b> <b>(".htmlspecialchars($row['authid'], ENT_QUOTES, 'UTF-8').")</b> был включен микрофон.", "green", $redir);
	}
	sb_list_action_done("Микрофон не включен", "Возникла ошибка включения микрофона <b>".htmlspecialchars(StripQuotes($row['name']), ENT_QUOTES, 'UTF-8')."</b>", "red", $redir);
}
else if(isset($_GET['a']) && $_GET['a'] == "delete")
{
	...
	{
		sb_list_action_done("Ошибка", "У вас нет доступа к этой операции.", "red", $redir);
	}
	...
	// Material Admin: ma_wb_unmute <type> <steam>; silence (3) → unmute both tracks
	$unmuteTypes = array();
	switch((int)$steam['type'])
	{
		case 1:
			$unmuteTypes = array(1);
			break;
	...
	if(empty($steam['RemoveType']) && ($length == 0 || $end > $now) && !empty($unmuteTypes))
	{
		...
		foreach($blocked as $tempban)
			{
				foreach ($unmuteTypes as $ut)
					SendRconSilent(("ma_wb_unmute " . $ut . " " . $steam['authid']), $tempban['sid']);
			}
	}
	...
	if($res)
	{
		...
		sb_list_action_done("Блокировка удалена", "Блокировка игрока <b>" . htmlspecialchars(StripQuotes($steam['name']), ENT_QUOTES, 'UTF-8') . "</b> (<b>".htmlspecialchars($steam['authid'], ENT_QUOTES, 'UTF-8')."</b>) была удалена из SourceBans.", "green", $redir);
	}
	sb_list_action_done("Блокировка НЕ удалена", "Блокировка игрока <b>".htmlspecialchars(StripQuotes($steam['name']), ENT_QUOTES, 'UTF-8')."</b> не удалена из-за ошибки.", "red", $redir);
}
...
if (isset($_GET['searchText']))
{
	...
	$res = $GLOBALS['db']->Execute(
		...
		(SELECT count(*) FROM ".DB_PREFIX."_comms as BH WHERE (BH.authid = CO.authid AND BH.authid != '' AND BH.authid IS NOT NULL AND BH.type IN (1,3))) as mute_count,
		(SELECT count(*) FROM ".DB_PREFIX."_comms as BH WHERE (BH.authid = CO.authid AND BH.authid != '' AND BH.authid IS NOT NULL AND BH.type IN (2,3))) as gag_count,
	...
	switch($type)
	{
		...
		case "admin":
            if($GLOBALS['config']['banlist.hideadminname']&&!$userbank->is_admin())
			{
                ...
                $bstatus = isset($_GET['bstatus']) ? $_GET['bstatus'] : '';
                switch($bstatus) {
                    case "active":
                        $where .= " AND CO.RemovedBy IS NULL AND (CO.length = 0 OR CO.ends > UNIX_TIMESTAMP())";
                        break;
                    case "expired":
                        $where .= " AND CO.RemovedBy IS NULL AND CO.length <> 0 AND CO.ends <= UNIX_TIMESTAMP()";
                        break;
                    case "removed":
                        $where .= " AND CO.RemovedBy IS NOT NULL";
                        break;
                    default:
                        $bstatus = '';
                        break;
                ...
                $theme->assign('bstatus_aid', $value);
            }
		break;
		case "where_banned":
			$where = "WHERE CO.sid=?";
...
while (!$res->EOF)
{
	...
	{
		case 1:
			$data['type_icon'] = sb_comms_type_icon_html(1, 20);
			$mute_count = $mute_count - 1;
			break;
		case 2:
			$data['type_icon'] = sb_comms_type_icon_html(2, 20);
			$gag_count = $gag_count - 1;
			break;
		case 3:
			$data['type_icon'] = sb_comms_type_icon_html(3, 20);
			$gag_count -= 1;
			$mute_count -= 1;
			break;
		default:
			$data['type_icon'] = '<img class="sb-ico" src="images/country/zz.gif" alt="Неизвестный тип блока" width="16" height="16" loading="lazy">';
			break;
	}
    ...
    if($history_count > 1)
        $data['prevoff_link'] = $history_count . " " . CreateLinkR("(найти)","index.php?p=commslist&searchText=" .$data['steamid']. "&Submit");
    else
        $data['prevoff_link'] = "Первая блокировка";

    $mutes = "";
    $gags = "";
    if($mute_count > 0)
    {
    	$mutes = $mute_count . '&thinsp;' . sb_comms_type_icon_html(1, 18);
    	if ($gag_count > 0)
    		$mutes = $mutes . "&ensp;";
    }
    if($gag_count > 0)
    	$gags = $gag_count . '&thinsp;' . sb_comms_type_icon_html(2, 18);
	...
	if (!empty($data['unbanned'])) {
		...
		if ($ubr === 'Истек' || stripos($ubr, 'истек') !== false)
			$data['status_kind'] = 'expired';
		elseif ($ubr === 'Удален' || stripos($ubr, 'удален') !== false)
			$data['status_kind'] = 'deleted';
		else
			$data['status_kind'] = 'unbanned';
	} elseif ((int)$res->fields['ban_length'] == 0) {
		$data['status_kind'] = 'perm';
	} elseif ((int)$res->fields['ban_length'] < 0) {
		$data['status_kind'] = 'session';
	} else {
		$data['status_kind'] = 'temp';
	}
...
if (function_exists('RecidivismAttachScoresToList'))
...
if($pages > 1) {
	...
	$ban_nav_p = ' / Страница: <span class="select" style="display: inline-block;"><select class="form-control" onchange="changePage(this,\'C\',' . $advSearchJs . ',' . $advTypeJs . ');" style="display: inline-block;width: 50px;">';
	for($i=1;$i<=$pages;$i++)
	{
...
if(isset($_GET["comment"])) {
	...
	if(isset($_GET["cid"])) {
        ...
        // SECURITY: escape stored comment text before it is reflected back into the edit form (stored XSS)
	...
	while(!$cotherdata->EOF)
	{
		...
		// SECURITY: escape stored comment text before HTML output (stored XSS) - matches page.banlist.php
