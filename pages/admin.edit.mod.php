...
if(isset($_POST['name']))
{
	...
	if($error == 0)
	{
		if($res['icon']!=$_POST['icon_hid'])
			...
			@unlink(SB_ICONS."/".basename($res['icon']));
		...
		$log = new CSystemLog("m", "МОД изменён", $userbank->GetProperty("user") . " изменил МОД (" . $name . ").");
	}
...
<div id="1">
<?php sb_ui_v2_theme_fragment('admin_edit_mod.twig'); ?>
<script>
