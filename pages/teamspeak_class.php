...
class teamspeakDisplayClass {
	...
	function displayTeamspeakEx($settings) {
		...
		if ($serverInfo["queryerror"] != 0) {
			...
			echo("}\n");
			echo("function tsAlert_" . $jsTeamspeakId . "(msg) {\n");
			echo("\tif (typeof ShowBox === 'function') { ShowBox('TeamSpeak', String(msg).replace(/\\n/g,'<br>'), 'red', '', true); return; }\n");
			echo("\tif (typeof sbSiteAlert === 'function') { sbSiteAlert(msg, 'TeamSpeak', 'red'); return; }\n");
			...
			echo("\t\ttsAlert_" . $jsTeamspeakId . "('Нельзя войти: в нике запрещённые символы: " . str_replace("'", "\\'", $settings["forbiddennicknamechars"]) . "');\n");
			echo("		return;\n");
			echo("	} else if (nickname == \"\") {\n");
			echo("\t\ttsAlert_" . $jsTeamspeakId . "('Нельзя войти: не указан ник.');\n");
			echo("		return;\n");
			echo("	}\n");
			...
			if ($serverInfo["serverinfo"]["server_password"] == "1") {
				...
				echo("\t\ttsAlert_" . $jsTeamspeakId . "('Нельзя войти: не указан пароль сервера.');\n");
				echo("		return;\n");
				echo("	}\n");
			...
			echo("\t\t\ttsAlert_" . $jsTeamspeakId . "('Нельзя войти: не указан пароль канала.');\n");
			echo("			return;\n");
			echo("		}\n");
