...
<?php
    ...
    if (isset($_POST['confirm']))
    {
		...
		if ($_POST['printonly'] > 0) {
			...
			$rs_tables = $db->query('SHOW TABLES') or die('Query failed.');
			while ($row_tables = $db->fetch_row($rs_tables))
			{
				...
				$rs = $db->query("SHOW FULL FIELDS FROM `$table` WHERE collation is not null AND collation <> 'utf8_general_ci'") or die('Query failed.');
			...
			$db->query("ALTER DATABASE `".DB_NAME."` DEFAULT CHARACTER SET $character_set COLLATE $convert_to;")or die('Query failed.');
			echo 'OK';
			$rs_tables = $db->query('SHOW TABLES') or die('Query failed.');
			while ($row_tables = $db->fetch_row($rs_tables))
			{
