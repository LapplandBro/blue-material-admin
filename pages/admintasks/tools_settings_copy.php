...
function scanCopyFiles($source,$dest) {
	...
	if (isset($_POST['confirm'])) {
		...
		echo '<ul><br />';
		check_writable();
		// Game codes are used unescaped both in raw SQL and as filesystem path segments below,
		// so restrict them to a safe charset (prevents SQL injection and path traversal via "../").
		$game1 = preg_replace('/[^A-Za-z0-9_\-]/', '', $game1);
		$game2 = preg_replace('/[^A-Za-z0-9_\-]/', '', $game2);
