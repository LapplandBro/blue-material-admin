		...
		function update ()
		{
			...
			foreach ($this->options as $opt)
			{
				if (($this->title == 'Fonts') || ($this->title == 'General')) {
					// SECURITY FIX: раньше здесь, в отличие от ветки valid_request() ниже, не
					// нейтрализовались кавычки, а $optval затем подставлялся без экранирования
					// прямо в UPDATE/INSERT (SQL-инъекция), если группа 'Fonts'/'General' будет
					// когда-либо (пере)добавлена. Добавлено экранирование через $db->escape().
