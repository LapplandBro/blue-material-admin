...
if (isset($_GET['a']) && $_GET['a'] == "unban" && isset($_GET['id']))
{
	...
	foreach($bids AS $bid) {
		...
		// Снять очки рецидива (ban-ветка)
		if (!empty($row['authid']))
			RecidivismRevokeOnUnpunish($row['authid'], 'ban', 'bans', $bid, 'web_unban');
...
if (isset($_GET['searchText']))
{
	...
	"SELECT BA.bid ban_id, BA.type, BA.ip ban_ip, BA.authid, BA.name player_name, created ban_created, ends ban_ends, length ban_length, reason ban_reason, BA.ureason unban_reason, BA.aid, AD.gid AS gid, adminIp, BA.sid ban_server, country ban_country, RemovedOn, RemovedBy, RemoveType row_type,
			SE.ip server_ip, AD.user admin_name, AD.comment admin_comm, AD.discord admin_discord, AD.vk admin_vk, AD.authid admin_authid, AD.gid, MO.icon as mod_icon,
...
//	}
	...
	if(function_exists('RecidivismCanView') && RecidivismCanView() && !empty($data['steamid']))
		$data['recidivism_link'] = CreateLinkR('Подробнее',"index.php?p=admin&c=recidivism&steam=".urlencode($data['steamid']));
	else
...
$ban_nav_p = '';
if($pages > 1) {
	$ban_nav_p = ' / Страница: <span class="select" style="display: inline-block;"><select class="form-control" onchange="changePage(this,\'B\',\''.(isset($_GET['advSearch']) ? $_GET['advSearch'] : '').'\',\''.(isset($_GET['advType']) ? $_GET['advType'] : '').'\');" style="display: inline-block;width: 50px;">';
	for($i=1;$i<=$pages;$i++)
	{
