...
// SECURITY FIX: this page was missing a permission check, allowing any admin who could reach the
// "servers" section (e.g. one with only ADMIN_LIST_SERVERS) to change a server's RCON password,
// IP/port and enabled state. Require explicit edit rights, matching sibling admin.edit.*.php pages.
...
if(isset($_POST['address']))
{
	...
	if($_POST['rcon'] != '*Скрыт*')
	{
		...
		// Пароль RCON в лог не пишем (чувствительные данные), только сам факт его смены.
		$log = new CSystemLog("m", "Сервер изменён", $userbank->GetProperty("user") . " изменил сервер (" . $ip . ":" . (int)$_POST['port'] . ")" . ($rcon_changed ? ", включая смену RCON-пароля" : "") . ".");
	}
}
