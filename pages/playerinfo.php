...
// Player Details
	...
	if (!$player && $uniqueid)
	{
		...
		// SECURITY FIX: $uniqueid уже проходит через valid_request() (htmlspecialchars с ENT_QUOTES),
		// что нейтрализует большинство SQL-инъекций через кавычки, но для надёжности дополнительно
		// экранируем значение через $db->escape() перед подстановкой в запрос.
...
// May get moved in the future
	...
	if ( $_GET['type'] == 'ajax' )
	{
		// SECURITY: original pattern '[^a-z]' had no regex delimiters and matched literally (only the
		// literal substring "a-z" at the very start of input), letting path traversal characters through
		// into the include() below (LFI). Use a proper delimited pattern; keep '_' since tab values like
		// "general_aliases" are split on it below.
		$tabs = explode('_', preg_replace('/[^a-z_]/', '', $_GET['tab']));
		foreach ( $tabs as $tab )
		{
