...
{
	$SteamID = trim(htmlspecialchars((string)($_POST['SteamID'] ?? '')));
	$BanIP = trim(htmlspecialchars((string)($_POST['BanIP'] ?? '')));
	$PlayerName = htmlspecialchars((string)($_POST['PlayerName'] ?? ''));
	$BanReason = htmlspecialchars((string)($_POST['BanReason'] ?? ''));
	$SubmitterName = htmlspecialchars((string)($_POST['SubmitName'] ?? ''));
	$Email = trim(htmlspecialchars((string)($_POST['EmailAddr'] ?? '')));
	...
	if ($validsubmit)
	{
		...
		if($demo || empty($_FILES['demo_file']['name']))
		{
			...
			foreach($admins AS $admin)
			{
				...
				$message .= "Игрок: ".$_POST['PlayerName']." (".$_POST['SteamID'].")\nДемо: ".(empty($_FILES['demo_file']['name'])?'Отсутствует':'Присутствует (http://' . sb_get_site_host() . $requri . 'getdemo.php?type=S&id='.$subid.')')."\n".$mailserver."Причина: ".$_POST['BanReason']."\n\n";
				$message .= "Кликните по ссылке для просмотра жалобы.\n\nhttp://" . sb_get_site_host() . $requri . "index.php?p=admin&c=bans#^2";
